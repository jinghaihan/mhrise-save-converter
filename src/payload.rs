    ...
    for value in &array.values {
        match value {
            ArrayValue::Array(value)
                if array.array_type == ARRAY_TYPE_VALUE
                    && array.member_type == FIELD_TYPE_ARRAY =>
            {
            ...
            ArrayValue::String(value)
                if array.array_type == ARRAY_TYPE_VALUE
                    && array.member_type == FIELD_TYPE_STRING =>
            {
                write_string(writer, value)?;
            }
...
impl<'a> Reader<'a> {
    ...
    fn align(&mut self, alignment: usize) -> Result<()> {
        ...
        self.offset =
            self.offset.checked_add(alignment - 1).context("offset overflow")? & !(alignment - 1);
...
impl Writer {
    fn align(&mut self, alignment: usize) {
        ...
        let len = self.data.len().div_ceil(alignment) * alignment;
...
mod tests {
    ...
    fn parses_and_reencodes_nested_payload() {
                    ...
                    fields: vec![
                        ...
                        Field {
                            ...
                            value: FieldValue::Array(Array {
                                ...
                                class_hashes: None,
                                values: vec![
                                    ArrayValue::Scalar(vec![1]),
                                    ArrayValue::Scalar(vec![2]),
                                ],
                            }),
                        },
                    ],
        ...
        let encoded = payload.encode().expect("fixture should encode");
        assert_eq!(SavePayload::parse(&encoded).expect("fixture should parse"), payload);
    }
}
