...
impl Citrus {
  ...
  pub fn new(steamid64: u64, param_index: Option<usize>) -> Self {
    Self { steamid64, param_index, d: Integer::from(!steamid64) }
  }
  ...
  fn aes_encrypt(buf: &mut [u8], key: [u8; 16], iv: [u8; 16]) -> &[u8] {
    ...
    Aes128CbcEnc::new(&key.into(), &iv.into()).encrypt_padded::<NoPadding>(buf, buf.len()).unwrap();

    &buf[..]
  }

  fn aes_decrypt(buf: &mut [u8], key: [u8; 16], iv: [u8; 16]) -> &[u8] {
    let pt = Aes128CbcDec::new(&key.into(), &iv.into()).decrypt_padded::<NoPadding>(buf).unwrap();
  ...
  fn ecc_encrypt_keys(
    ...
    out[0..128].copy_from_slice(&Self::encrypt_key_segment(&k0, key_lo, pub_key.clone(), params)?);
    ...
    out[256..384].copy_from_slice(&Self::encrypt_key_segment(&k2, iv_lo, pub_key.clone(), params)?);
    out[384..512].copy_from_slice(&Self::encrypt_key_segment(&k3, iv_hi, pub_key.clone(), params)?);
    Some(out)
  }
  ...
  fn ecc_decrypt_keys(
    ...
    let mut key = [0u8; 16];
    let mut iv = [0u8; 16];
    key[0..8].copy_from_slice(&self.decrypt_key_segment(&buf[0..128], params)?.to_u64_le_bytes());
    key[8..16]
      .copy_from_slice(&self.decrypt_key_segment(&buf[128..256], params)?.to_u64_le_bytes());
    iv[0..8].copy_from_slice(&self.decrypt_key_segment(&buf[256..384], params)?.to_u64_le_bytes());
    iv[8..16].copy_from_slice(&self.decrypt_key_segment(&buf[384..512], params)?.to_u64_le_bytes());
    Some((key, iv))
  }
