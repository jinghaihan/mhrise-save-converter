...
impl Citrus {
    ...
    fn aes_decrypt(buf: &mut [u8], key: [u8; 16], iv: [u8; 16]) -> &[u8] {
        let pt =
            Aes128CbcDec::new(&key.into(), &iv.into()).decrypt_padded::<NoPadding>(buf).unwrap();
    ...
    fn ecc_decrypt_keys(
        ...
        let mut iv = [0u8; 16];
        key[0..8]
            .copy_from_slice(&self.decrypt_key_segment(&buf[0..128], params)?.to_u64_le_bytes());
        key[8..16]
            .copy_from_slice(&self.decrypt_key_segment(&buf[128..256], params)?.to_u64_le_bytes());
        iv[0..8]
            .copy_from_slice(&self.decrypt_key_segment(&buf[256..384], params)?.to_u64_le_bytes());
        iv[8..16]
            .copy_from_slice(&self.decrypt_key_segment(&buf[384..512], params)?.to_u64_le_bytes());
        Some((key, iv))
    }
