...
Rust formatting uses two-space indentation as configured in `rustfmt.toml`. Pushes and pull requests run checks and build Linux, macOS, and Windows packages. Tagged releases are generated by GitHub Actions with [changelogithub](https://github.com/antfu/changelogithub), attached archives, and SHA-256 files.

To publish a release from a clean `main` branch:

```bash
python3 tools/release.py --current --yes   # publish the current version
python3 tools/release.py --bump patch      # publish the next patch version
```
